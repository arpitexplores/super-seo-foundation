...
```bash
head -5 tool-specific skills path/seo/SKILL.md
```
Should start with `---` followed by YAML.

3. Restart AI coding assistant:
```bash
agent-cli
```
...
```bash
ls tool-specific skills path/seo/requirements.txt
```

If missing, download it directly:
```bash
curl -fsSL https://raw.githubusercontent.com/AgriciDaniel/seo-toolkit/main/requirements.txt \
  -o tool-specific skills path/seo/requirements.txt
```
...
```bash
ls tool-specific config pathsagents/seo-*.md
```

2. Check agent frontmatter:
```bash
head -5 tool-specific config pathsagents/seo-technical.md
```

3. Re-install agents:
```bash
cp /path/to/seo-toolkit/agents/*.md tool-specific config pathsagents/
```
...
3. **Logs:** Check AI coding assistant's output for error details

## Debug Mode

To see detailed output, check AI coding assistant's internal logs or run scripts directly:

```bash
# Test fetch
python3 tool-specific skills path/seo/scripts/fetch_page.py https://example.com

# Test parse
python3 tool-specific skills path/seo/scripts/parse_html.py page.html --json

# Test screenshot
python3 tool-specific skills path/seo/scripts/capture_screenshot.py https://example.com
```
