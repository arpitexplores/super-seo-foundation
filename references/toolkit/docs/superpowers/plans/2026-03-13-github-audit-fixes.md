...
- [ ] Verify the JSON is valid: `python3 -c "import json; json.load(open('plugin manifest'))"`
- [ ] Commit:
  ```bash
...
**What & Why:**
- H1 is `# AI assistant SEO` (weak keyword signal) → add keyword suffix
...
- [ ] Strengthen H1 (line 5):
  - Change `# AI assistant SEO` -> `# AI assistant SEO: SEO Audit Skill for AI coding assistant`
- [ ] Add version badge after existing badges (line 12, after License badge):
  ```markdown
  [![Version](https://img.shields.io/github/v/release/AgriciDaniel/seo-toolkit)](https://github.com/AgriciDaniel/seo-toolkit/releases)
  ```
...
- [ ] Fix architecture block (line 181 after ToC insertion - find by content):
  - Change `tool-specific config pathsagents/seo-*.md     # Subagents (6 total)` → `tool-specific config pathsagents/seo-*.md     # Subagents (7 total)`
...
- [ ] Create `CITATION.cff` at repo root:
  ...
  repository-code: 'https://github.com/AgriciDaniel/seo-toolkit'
  url: 'https://github.com/AgriciDaniel/seo-toolkit'
  license: MIT
  version: 1.4.0
...
- [ ] Create `.github/ISSUE_TEMPLATE/bug_report.yml`:
  ```yaml
  name: Bug Report
  description: Report a bug or unexpected behavior in AI assistant SEO
  title: "[Bug]: "
  labels: ["bug"]
  body:
    - type: markdown
...
- [ ] Create `.github/ISSUE_TEMPLATE/feature_request.yml`:
  ...
  description: Suggest a new feature or improvement for AI assistant SEO
  title: "[Feature]: "
  labels: ["enhancement"]
  body:
    - type: markdown
      attributes:
        value: |
          Have a great idea? We'd love to hear it! For general discussion, consider using [GitHub Discussions](https://github.com/AgriciDaniel/seo-toolkit/discussions) instead.
    - type: textarea
      id: problem
      attributes:
...
- [ ] Create `.github/ISSUE_TEMPLATE/config.yml`:
  ...
  contact_links:
    - name: Question or Discussion
      url: https://github.com/AgriciDaniel/seo-toolkit/discussions
...
- [ ] Enable Discussions:
  ```bash
  gh repo edit AgriciDaniel/seo-toolkit --enable-discussions
  ```
  Verify: `gh repo view AgriciDaniel/seo-toolkit --json hasDiscussionsEnabled --jq .hasDiscussionsEnabled`
  Expected: `true`

- [ ] Disable Wiki:
  ```bash
  gh repo edit AgriciDaniel/seo-toolkit --enable-wiki=false
  ```
  Verify: `gh repo view AgriciDaniel/seo-toolkit --json hasWikiEnabled --jq .hasWikiEnabled`
  Expected: `false`

- [ ] Fix description (replace "6 subagents" with "7 subagents"):
  ```bash
  gh repo edit AgriciDaniel/seo-toolkit --description "Universal SEO skill for AI coding assistant. 13 sub-skills, 7 subagents, extensions system with DataForSEO MCP integration. Technical SEO, E-E-A-T, schema, GEO/AEO, and strategic planning."
  ```
  Verify: `gh repo view AgriciDaniel/seo-toolkit --json description --jq .description`

- [ ] Add topics `python` and `skill`:
  ```bash
  gh repo edit AgriciDaniel/seo-toolkit --add-topic python --add-topic skill
  ```
  Verify: `gh repo view AgriciDaniel/seo-toolkit --json repositoryTopics --jq '.repositoryTopics | map(.name)'`
...
- [ ] Confirm plugin.json is valid JSON and has 7 agents:
  ```bash
  python3 -c "import json; d=json.load(open('plugin manifest')); print(d['version'], len(d['agents']))"
  ```
  Expected: `1.4.0 7`
...
- [ ] Confirm no stale "6 subagent/total" strings remain in actively-versioned content:
  ...
  grep -rn "6 subagent\|subagents (6\|6 total" --include="*.md" --include="*.json" . \
    --exclude-dir=".git" --exclude-dir="seo-toolkit-main" --exclude-dir="ads-toolkit-main"
  ```
  Expected: Only line 123 in CHANGELOG.md (v1.0.0 historical entry, correct, do not touch)

- [ ] Confirm GitHub settings applied:
  ```bash
  gh repo view AgriciDaniel/seo-toolkit --json description,hasDiscussionsEnabled,hasWikiEnabled,repositoryTopics
  ```

- [ ] Confirm CI workflow is visible on GitHub:
  ```bash
  gh workflow list --repo AgriciDaniel/seo-toolkit
  ```
...
- `AGENT_CONTEXT.md`: already correctly says "7 parallel subagents"
