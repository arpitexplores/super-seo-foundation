...
add to `tool-specific config pathssettings.json`:
...
```bash
python3 tool-specific skills path/seo-image-gen/scripts/validate_setup.py
```

Or check manually:
1. `ls tool-specific skills path/seo-image-gen/SKILL.md`:skill file exists
2. `ls tool-specific config pathsagents/seo-image-gen.md`:agent file exists
3. `grep nanobanana tool-specific config pathssettings.json`:MCP configured

## Common Issues

### "MCP tools not available"
- Restart AI coding assistant after installing the extension
- Verify your API key is valid at [aistudio.google.com](https://aistudio.google.com)
- Check `tool-specific config pathssettings.json` has the nanobanana-mcp entry
...
- AI assistant will suggest rephrased alternatives automatically
...
- Check the path returned by AI assistant after generation
