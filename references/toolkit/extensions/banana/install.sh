...
# Banana Image Generation Extension Installer for SEO toolkit
# Wraps everything in main() to prevent partial execution on network failure

main() {
    SKILL_DIR="${HOME}/.agent/skills/seo-image-gen"
    ...
    if [ -f "${SCRIPT_DIR}/skills/seo-image-gen/REFERENCE.md" ]; then
        SOURCE_DIR="${SCRIPT_DIR}"
    elif [ -f "${SCRIPT_DIR}/extensions/banana/skills/seo-image-gen/REFERENCE.md" ]; then
        SOURCE_DIR="${SCRIPT_DIR}/extensions/banana"
    else
        echo "✗ Cannot find extension source files."
        echo "  Run this script from the seo-toolkit repo: ./extensions/banana/install.sh"
        exit 1
    fi
...
" || {
    ...
    mkdir -p "${SKILL_DIR}"
    cp "${SOURCE_DIR}/skills/seo-image-gen/REFERENCE.md" "${SKILL_DIR}/SKILL.md"
