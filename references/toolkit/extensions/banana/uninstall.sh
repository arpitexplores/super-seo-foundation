...
main() {
    ...
    rm -rf "${HOME}/.agent/skills/seo-image-gen"

    # Remove agent
    rm -f "${HOME}/.agent/agents/seo-image-gen.md"
    ...
    if [ -f "${SETTINGS_FILE}" ]; then
        ...
        if [ -d "${HOME}/.agent/skills/banana" ]; then
            echo "  ℹ  Standalone banana skill detected at tool-specific config pathsskills/banana/"
...
" 2>/dev/null || echo "  ⚠  Could not auto-remove MCP config. Remove 'nanobanana-mcp' from tool-specific config pathssettings.json manually."
        fi
    fi
