...
# DataForSEO Extension Installer for SEO toolkit
# Wraps everything in main() to prevent partial execution on network failure

main() {
    SKILL_DIR="${HOME}/.agent/skills/seo-dataforseo"
    AGENT_DIR="${HOME}/.agent/agents"
    SEO_SKILL_DIR="${HOME}/.agent/skills/seo"
    SETTINGS_FILE="${HOME}/.agent/settings.json"

    echo "════════════════════════════════════════"
    echo "║   DataForSEO Extension - Installer   ║"
    echo "║   For SEO toolkit                     ║"
    echo "════════════════════════════════════════"
    echo ""

    # Check prerequisites
    if [ ! -d "${SEO_SKILL_DIR}" ]; then
        echo "✗ SEO toolkit is not installed."
        echo "  Install it first: curl -fsSL https://raw.githubusercontent.com/AgriciDaniel/seo-toolkit/main/install.sh | bash"
        exit 1
    fi
    echo "✓ SEO toolkit detected"
    ...
    if [ -f "${SCRIPT_DIR}/skills/seo-dataforseo/REFERENCE.md" ]; then
        SOURCE_DIR="${SCRIPT_DIR}"
    elif [ -f "${SCRIPT_DIR}/extensions/dataforseo/skills/seo-dataforseo/REFERENCE.md" ]; then
        SOURCE_DIR="${SCRIPT_DIR}/extensions/dataforseo"
    else
        echo "✗ Cannot find extension source files."
        echo "  Run this script from the seo-toolkit repo: ./extensions/dataforseo/install.sh"
        exit 1
    fi
    ...
    mkdir -p "${SKILL_DIR}"
    cp "${SOURCE_DIR}/skills/seo-dataforseo/REFERENCE.md" "${SKILL_DIR}/SKILL.md"
...
" || {
        ...
        echo "  Add the dataforseo server manually to tool-specific config pathssettings.json"
    ...
    echo ""
    echo "Usage:"
    echo "  1. Start your AI assistant CLI"
    echo "  2. Run commands:"
