#!/usr/bin/env python3
"""Post-edit schema validation hook for AI coding assistant.

Validates JSON-LD schema after file edits. Returns exit code 2 to block
if critical validation errors found.

Hook configuration in tool-specific config pathssettings.json:
{
  "hooks": {
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "python3 tool-specific config pathsskills/seo/hooks/validate-schema.py \"$FILE_PATH\"",
            "exitCodes": { "2": "block" }
          }
        ]
      }
