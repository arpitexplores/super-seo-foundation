#!/usr/bin/env python3
"""
Google API credential management for Super SEO.
...
CONFIG_PATH = os.path.expanduser("~/.config/super-seo/google-api.json")
TOKEN_PATH = os.path.expanduser("~/.config/super-seo/oauth-token.json")
...
def load_config() -> dict:
    ...
    Reads ~/.config/super-seo/google-api.json first. Any missing fields
...
6. CREATE CONFIG FILE
   mkdir -p ~/.config/super-seo
   Save to ~/.config/super-seo/google-api.json:
...
def main():
    parser = argparse.ArgumentParser(
        description="Google API credential management for Super SEO"
    )
    parser.add_argument(
        "--check",
        nargs="?",
